...
export default class GenericDialog extends React.Component {
    constructor(props, context) {
        ...
        this.abortType=this.props.value.type;
    ...
    onChangeAttr = (field,attr,newidx,value) => {
    ...
    render() {
        ...
        return (<Dialog header={"Edit " + this.props.value.type} position="center" visible={this.props.display} style={{ width: '50vw' }} modal={true} footer={footer} onHide={this.onCancelDialog} className={"generic " + this.props.value.type}>
          ...
          <div className='inputs'>
            <div className='input'>
              {attr && attr.type === 'string' && (
                <InputText value={attr.value} onChange={(e) => this.onChangeAttr('value', attr,idx,e.target.value)}/>
              )}
              {attr && attr.type === 'number' && (
                <InputNumber className='inputint' onChange={(e) => this.onChangeAttr('value', attr,idx,e.target.value)}
                mode="decimal" inputMode='decimal' minFractionDigits={1} maxFractionDigits={5} min={0} useGrouping={false}
                value={attr.value}></InputNumber>
              )}
              {attr && attr.type === 'int' && (
                <InputNumber className='inputint' onChange={(e) => this.onChangeAttr('value', attr,idx,e.target.value)}
                mode="decimal" useGrouping={false} value={attr.value}></InputNumber>
              )}
              {attr && attr.type === 'year' && (
                <InputNumber className='inputint' onChange={(e) => this.onChangeAttr('value', attr,idx,e.target.value)}
                min={1900} max={2100} mode="decimal" useGrouping={false} value={attr.value}></InputNumber>
              )}
              {attr && attr.type === 'date' && (
                <Calendar appendTo={document.body} onChange={(e) => this.onChangeAttr('value', attr,idx,e.target.value)}
                dateFormat="yy-mm-dd" value={new Date(attr.value)}></Calendar>
              )}
              {attr && attr.type === 'enum' && (
                <Dropdown appendTo={document.body} onChange={(e) => this.onChangeAttr('value', attr,idx,e.value)} options={this.enums[attr.name]} value={attr.value}></Dropdown>
              )}
            </div>
          </div>
        </div>
